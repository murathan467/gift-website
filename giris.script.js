document.getElementById('giris').addEventListener('click', function() {
    ...
    const kullanıcıAdı = document.getElementById('kullanıcıAdı').value.trim();
    const eposta = document.getElementById('eposta').value.trim();
    const parola = document.getElementById('parola').value.trim();
    const telefon = document.getElementById('stelefon').value.trim();
    const mesajDiv = document.getElementById('mesaj');

    if (!isim || !soyisim || !kullanıcıAdı || !eposta || !parola || !telefon) {
        mesajDiv.style.color = 'red';
        mesajDiv.textContent = 'Eksik bölümleri lütfen doldurunuz.';
        return;
    }

    if (isim.toLowerCase() === '****' || isim.toLowerCase() === '******') {
        if (soyisim.toLowerCase() === '*********') {
    ...
    mesajDiv.style.color = 'green';
    mesajDiv.textContent = 'Kaydınız başarı ile oluşturuldu.Lütfen giriş tuşuna basın.';
});
